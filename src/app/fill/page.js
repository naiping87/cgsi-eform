    ...
    <main style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      <div style={{display:'flex',height:'100vh'}}>
        <div style={{flex:1,overflow:'auto',background:'#525659',position:'relative',padding:16}}>
          <div style={{marginBottom:10,display:'flex',gap:6,alignItems:'center'}}>
            ...
            <span style={{color:'#ccc',fontSize:11,marginLeft:'auto'}}>
              {activeField ? `Click where "${activeField}" goes` : 'Select field → click PDF'}
            </span>
          </div>
          ...
          <div style={{position:'relative',display:'inline-block',maxWidth:'100%'}}>
            ...
            {pageSize && Object.entries(positions).filter(([_,p])=>p.page===pageNum).map(([key,pos])=>{
              ...
              return (
                <div key={key} style={{position:'absolute',left:mx-5,top:my-5,width:10,height:10,
                  borderRadius:'50%',background:'var(--success)',border:'2px solid #fff',zIndex:10}}
        ...
        <div style={{width:320,background:'var(--bg-card)',borderLeft:'1px solid var(--border)',padding:16,overflowY:'auto'}}>
          ...
          <p className="text-caption" style={{marginBottom:12}}>1. Type value &nbsp; 2. Click field name &nbsp; 3. Click PDF to place</p>

          {template.fields.map((field) => {
            const pos = positions[field.key];
            const isActive = activeField === field.key;
            return (
              <div key={field.key} style={{marginBottom:8,padding:8,borderRadius:8,
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                  ...
                  {pos && <button onClick={()=>removePosition(field.key)}
                ...
                <button onClick={()=>setActiveField(isActive?null:field.key)}
                  style={{marginTop:4,width:'100%',padding:'4px',borderRadius:4,
                  ...
                  {isActive?'Click on PDF ↑':pos?`✓ (${Math.round(pos.x)},${Math.round(pos.y)})`:'Set position'}
                </button>
              </div>
            );
          })}
          ...
          <p style={{fontSize:10,color:'var(--text-muted)',textAlign:'center',marginTop:6}}>
            Click positions saved in browser.
          </p>
        </div>
      </div>
    </main>
